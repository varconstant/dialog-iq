...
const PERSONAL_TOKEN = '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'

export const getReleasedFiles = async (toolName: string, platform: string): Promise<ProcessedRelease[]> => {
  try {
    const platformMatch = new RegExp(`.*-(${platform.replace('win32', 'win')})-x64`)

    const { data } = await axios.get<GithubRelease[]>(toolsList[toolName].url, {headers: { Authorization: 'Bearer '+ PERSONAL_TOKEN }})
